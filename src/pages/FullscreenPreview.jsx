...
import { ArrowLeft, Loader2, AlertCircle, Globe2, QrCode, Infinity as InfinityIcon, LockKeyhole, X, Heart, CheckCircle2, Wallet } from 'lucide-react';
...
const FullscreenPreview = () => {
  ...
  const TemplateComponent = pageData ? (TEMPLATE_COMPONENTS[templateId] || TEMPLATE_COMPONENTS[DEFAULT_TEMPLATE_ID]) : null;
  ...
  const publishWithoutPayment = async (method = 'instant') => {
    ...
    try {
      ...
      if (raw.includes('not-found') || raw.includes('failed-precondition') || raw.includes('unavailable')) {
        try {
          await updateDoc(doc(db, 'pages', draftId), {
            ...
            checkoutOwnerEmail: user?.email || '',
            checkoutStartedAt: serverTimestamp(),
          });
          trackEvent('publish_skipped_payment_client_fallback', { draftId, templateId: pageData?.templateId, method });
          setPublishModalOpen(false);
          navigate(`/success?draft=${draftId}&test=1&method=${method}`);
        } catch (fallbackErr) {
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50">
        ...
        <div className="min-h-24 bg-[#fffdf8] border-t border-[#eadfca] shadow-[0_-8px_30px_rgba(89,62,27,0.12)] px-6 md:px-12 py-4 flex items-center justify-between gap-4">
          <div className="max-w-[360px]">
            ...
            <p className="text-xs text-[#876f5a]">Open the publish card and make it live instantly for testing.</p>
          </div>
          <button
      ...
      {publishModalOpen && (
        <div className="fixed inset-0 z-[80] bg-black/55 backdrop-blur-[2px] flex items-center justify-center p-4">
          <div className="w-full max-w-[560px] rounded-[30px] bg-white shadow-[0_28px_80px_rgba(20,10,35,0.45)] border border-[#f1e7eb] p-5 md:p-7 relative">
            ...
            <h2 className="text-center text-[2.45rem] md:text-[2.8rem] font-playfair font-bold text-[#131a2f] leading-[0.95] mb-2">Publish Your Gift</h2>
            <p className="text-center text-[#6b7280] text-[1.02rem] md:text-[1.12rem] mb-4">Unlock your custom "{templateTitle}"</p>

            <div className="rounded-3xl border border-[#f0dbe4] bg-[#fbf3f7] p-4 md:p-5 mb-4">
              <p className="text-[1.55rem] md:text-[1.75rem] font-playfair font-bold text-[#20283a] mb-2.5">What you get forever:</p>
              <div className="space-y-2 text-[#4b5563] text-[0.98rem] md:text-[1.06rem]">
                {[
            ...
            <div className="rounded-3xl border border-[#e7e7ea] bg-[#f9f9fb] px-4 md:px-5 py-3.5 md:py-4 mb-4 flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[2.3rem] md:text-[2.55rem] leading-none font-black text-[#111827]">{displayPrice}</span>
                  <span className="text-[1.55rem] text-[#9ca3af] line-through">$12.00</span>
                  <span className="text-[0.95rem] font-bold text-[#e3627f] bg-[#ffe3ea] px-2 py-0.5 rounded-full">Save 33%</span>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              >
                <Wallet size={20} />
                {publishing ? 'Publishing...' : 'Instant Publish (Test Mode)'}
              </button>
              <button
                type="button"
                onClick={() => publishWithoutPayment('local')}
                disabled={publishing}
                className="w-full rounded-full bg-[#081538] text-white text-[1.08rem] md:text-[1.16rem] font-bold py-3.5 shadow-[0_10px_26px_rgba(8,21,56,0.35)] disabled:opacity-70 flex items-center justify-center gap-2"
              >
                <Wallet size={20} />
                {publishing ? 'Publishing...' : 'Local Publish (Test Mode)'}
              </button>
            </div>
